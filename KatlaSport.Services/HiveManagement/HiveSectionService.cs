...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        /// <inheritdoc/>
        public async Task<HiveSection> CreateHiveAsync(UpdateHiveSectionRequest createRequest)
        {
            var dbHives = _context.Sections.Where(h => h.Code == createRequest.Code).ToArray();
            if (dbHives.Length > 0)
            {
        ...
        /// <inheritdoc/>
        public async Task<HiveSection> UpdateHiveAsync(int hiveSectionId, UpdateHiveSectionRequest updateRequest)
        {
            var dbHiveSections = _context.Sections.Where(p => p.Code == updateRequest.Code && p.Id != hiveSectionId).ToArray();
            if (dbHiveSections.Length > 0)
            {
                throw new RequestedResourceHasConflictException("code");
            }
        ...
        {
            var dbHiveSections = _context.Sections.Where(p => p.Id == hiveSectionId).ToArray();
            if (dbHiveSections.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
